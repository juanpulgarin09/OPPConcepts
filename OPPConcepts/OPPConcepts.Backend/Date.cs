...
public class Date
{
    ...
    private int ValidateMonth(int month)
    {
        if (month < 1 || month > 12)
        {
            throw new ArgumentOutOfRangeException(nameof(month), "Year must be between 1 and 12.");
        }
        return month;
    }
    ...
    private int ValidateDay(int day)
    {
        if (day < 1)
        {
            throw new ArgumentOutOfRangeException(nameof(day), "Year must be greathes than 0.");
        }
